...
// ── Secciones async (cada una streamea con su propio skeleton) ──────

async function CategoriesSection() {
  const categories = await apiFetch<CategoryDto[]>('/categories').catch(
    () => [] as CategoryDto[],
  );
        ...
        {categories.map((cat) => (
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-zinc-400">{cat.children.length} rubros</p>
            </div>
          </Link>
        ))}
      </div>
...
async function BestSellers() {
  const [bestSellers, favoriteIds] = await Promise.all([
...
async function RecentProducts() {
  const [recent, favoriteIds] = await Promise.all([
